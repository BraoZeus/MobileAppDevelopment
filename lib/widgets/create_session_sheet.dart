...
class _CreateSessionSheetState extends State<CreateSessionSheet> {
  ...
  Future<void> _pickExamDate() async {
    ...
    if (time == null) return;
    setState(() => _selectedExamDate = DateTime(picked.year, picked.month, picked.day, time.hour, time.minute));
  }
  ...
  void _submit() {
    ...
    if (subject.isEmpty) {
      setState(() => _errorMessage = 'Please enter a subject name.');
      return;
    }
    if (_hasExam && _selectedExamDate == null) {
      setState(() => _errorMessage = 'Please select an exam date.');
      return;
    }
          ...
          child: Column(
            ...
            children: [
              ...
              if (_errorMessage != null) ...[
                Text(_errorMessage!, style: const TextStyle(color: Colors.redAccent, fontSize: 13, fontWeight: FontWeight.bold)),
                              ...
                              Expanded(
                                child: Text(
                                  _selectedExamDate == null
                                      ? 'Select Exam Time'
                                      : 'Target: ${DateFormat('MMM dd, yyyy h:mm a').format(_selectedExamDate!)}',
                                  style: TextStyle(
